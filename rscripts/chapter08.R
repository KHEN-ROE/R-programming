...
# 실습: histogram()  함수를 이용하여 데잍 시각화하기  #안뜨면 다시 실행해보면됨
histogram(~gcsescore, data = Chem97) # ~ 연산자? ...
#그래프를 그릴 때는 x,y데이터가 필요함
#데이터가 있던가, 계산할 수 있던가
#데이터는 그릴 수 있는 형태로 있어야함
#Chem97 안에있는 gcsescore이용해서 히스토그램 그려달라
...
# 단계 4: 막대 그래프 그리기 
barchart(Var1 ~ Freq | Var2, data = dft, layout = c(4, 1)) #1행 4열로 나온다. 컬럼중심이라서


# 단계 5: origin 속성을 사용하여 막대 그래프 그리기 # ~는 연계데이터라는 의미 
barchart(Var1 ~ Freq | Var2, data = dft, layout = c(4, 1), origin = 0) # var1 freq를 x,y로 놓고 var2를 추가변수로 줌
#R은 파라미터에 이름 붙일 수 있다. data를 dft로 네이밍
...
xyplot(lat ~ long | magnitudegroup, data = quakes, #sql에서 Select, where, from 절이라고 생각
